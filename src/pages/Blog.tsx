  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 20px" }}>
      <button onClick={onBack} style={{
        ...
        color: "#7c3aed", fontSize: 14, fontWeight: 500, padding: 0, marginBottom: 32,
      }}>← Back to Blog</button>
      ...
      <h1 style={{
        fontSize: 36, fontWeight: 800, lineHeight: 1.2, color: "#111827",
        margin: "0 0 16px", fontFamily: "'Georgia', serif",
      }}>{post.title}</h1>

      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28, color: "#6b7280", fontSize: 14 }}>
        <div style={{
          width: 32, height: 32, borderRadius: "50%", background: "#7c3aed", color: "white",
      ...
      {post.coverImage && (
        <img src={post.coverImage} alt={post.title} style={{
          width: "100%", maxHeight: 380, objectFit: "cover", borderRadius: 12, marginBottom: 36,
        }} />
      )}

      <div
        dangerouslySetInnerHTML={{ __html: post.content }}
        style={{ fontSize: 17, lineHeight: 1.85, color: "#1f2937", fontFamily: "'Georgia', serif" }}
        className="blog-content"
      />

      <div style={{ marginTop: 56, paddingTop: 24, borderTop: "1px solid #e5e7eb" }}>
        <button onClick={onBack} style={{
          background: "none", border: "1px solid #e5e7eb", borderRadius: 8,
          padding: "8px 18px", cursor: "pointer", color: "#374151", fontSize: 14,
        }}>← More Posts</button>
      </div>
    </div>
  );
  ...
  if (posts.length === 0) {
    return (
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "80px 20px", textAlign: "center" }}>
        ...
        <h2 style={{ color: "#374151" }}>No posts yet</h2>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 20px" }}>
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 16, margin: 0 }}>Thoughts, tutorials, and updates.</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 32, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {["All", ...allTags].map(tag => (
            <button key={tag} onClick={() => setActiveTag(tag === "All" ? null : tag)} style={{
              padding: "6px 14px", borderRadius: 6, border: "1px solid",
              borderColor: (tag === "All" ? !activeTag : activeTag === tag) ? "#a78bfa" : "#e5e7eb",
              background: (tag === "All" ? !activeTag : activeTag === tag) ? "#ede9fe" : "white",
              color: (tag === "All" ? !activeTag : activeTag === tag) ? "#7c3aed" : "#6b7280",
      ...
      {filtered.length === 0 ? (
        <p style={{ color: "#9ca3af", textAlign: "center", padding: "40px 0" }}>No matching posts found.</p>
      ) : (
        <>
          {featured && (
            <div onClick={() => onSelect(featured)} style={{
              background: "white", border: "1px solid #e5e7eb", borderRadius: 16,
              overflow: "hidden", cursor: "pointer", marginBottom: 28, transition: "box-shadow 0.2s",
            }}
              onMouseEnter={e => (e.currentTarget.style.boxShadow = "0 4px 20px rgba(124,58,237,0.1)")}
              onMouseLeave={e => (e.currentTarget.style.boxShadow = "none")}
            >
              {featured.coverImage && (
                <img src={featured.coverImage} alt={featured.title} style={{ width: "100%", height: 260, objectFit: "cover" }} />
              )}
          ...
          {rest.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
              {rest.map(post => (
                ...
                >
                  {post.coverImage && (
                    <img src={post.coverImage} alt="" style={{ width: "100%", height: 140, objectFit: "cover" }} />
                  )}
...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      .blog-content h1,.blog-content h2,.blog-content h3{font-family:Georgia,serif;font-weight:700;margin:1.5em 0 0.5em;color:#111827}
      .blog-content h1{font-size:28px}.blog-content h2{font-size:22px}.blog-content h3{font-size:18px}
      .blog-content p{margin:0 0 1.2em}
      .blog-content blockquote{border-left:3px solid #7c3aed;margin:1.5em 0;padding:12px 20px;background:#faf5ff;color:#374151;font-style:italic;border-radius:0 8px 8px 0}
      .blog-content pre{background:#1f2937;color:#f9fafb;padding:16px 20px;border-radius:8px;overflow-x:auto;font-size:14px;line-height:1.7;margin:1.5em 0}
  ...
  if (loading) {
    return (
      <div style={{ textAlign: "center", padding: "100px 20px", color: "#9ca3af", fontSize: 16 }}>
        Loading posts...
      </div>
    );
  }
